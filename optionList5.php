...
<section class="index">
	...
	<section class="optionList">
		...
		<div>
			...
			$query = "SELECT * FROM purpose_and_program WHERE purpose = '영상/음향작업용'";
			$result = mysqli_query($conn, $query);
			...
			$query = "SELECT * FROM purpose_and_program WHERE purpose = '영상/음향작업용' ORDER BY purpose DESC LIMIT $start_point, $list"; //첫번째 게시글부터 한페이지에 보여줄 게시글 갯수만큼 들고옴
			$result = mysqli_query($conn, $query);
