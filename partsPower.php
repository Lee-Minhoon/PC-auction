...
<section class="index">
	...
	<section class="partsList">
		...
		<div>
			...
			<div class="block2"><p class="head">파워 형식</p></div>
			<div class="block3"><p class="head">정격 출력</p></div><br>
			<?php
			$conn = db_connect();
			...
			$query = "SELECT * FROM power ORDER BY parts_num DESC LIMIT $start_point, $list"; //첫번째 게시글부터 한페이지에 보여줄 게시글 갯수만큼 들고옴
			$result = mysqli_query($conn, $query);
			...
			?>
					<div class="perLine">
						...
						<div class="block2"><?= $rows['power_form'] ?></div>
						<div class="block3"><?= $rows['rated_output'] ?></div>
					</div>
			<?php
				}
			}
