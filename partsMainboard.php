...
<section class="index">
	...
	<section class="partsList">
		<div style="width: 992px; height:50px; border-bottom: 1px solid #DEE3EB;">
		...
		<div>
			...
			<div class="block2"><p class="head">규격</p></div>
			<div class="block3"><p class="head">소켓</p></div>
			<div class="block4"><p class="head">메모리 종류</p></div>
			<div class="block5"><p class="head">메모리 슬롯</p></div>
			<div class="block6"><p class="head">용량</p></div><br>
			<?php
			$conn = db_connect();
			...
			$query = "SELECT * FROM mainboard ORDER BY parts_num DESC LIMIT $start_point, $list"; //첫번째 게시글부터 한페이지에 보여줄 게시글 갯수만큼 들고옴
			$result = mysqli_query($conn, $query);
			...
			?>
					<div class="perLine">
						...
						<div class="block2"><?= $rows['std'] ?></div>
						<div class="block3"><?= $rows['socket'] ?></div>
						<div class="block4"><?= $rows['main_mem_kind'] ?></div>
						<div class="block5"><?= $rows['main_mem_slot'] ?></div>
						<div class="block6"><?= $rows['vga_slot'] ?></div>
					</div>
			<?php
				}
			}
