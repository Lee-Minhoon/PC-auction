...
<section class="index">
	...
	<section class="partsList">
		...
		<div>
			...
			<div class="block2"><p class="head">인터페이스</p></div>
			<div class="block3"><p class="head">벤치마크 점수</p></div><br>
			<?php
			$conn = db_connect();
			...
			$query = "SELECT * FROM vga ORDER BY parts_num DESC LIMIT $start_point, $list"; //첫번째 게시글부터 한페이지에 보여줄 게시글 갯수만큼 들고옴
			$result = mysqli_query($conn, $query);
			...
			?>
					<div class="perLine">
						...
						<div class="block2"><?= $rows['interface'] ?></div>
						<div class="block3"><?= $rows['vga_bm_score'] ?></div>
					</div>
			<?php
				}
			}
