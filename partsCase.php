...
<section class="index">
	...
	<section class="partsList">
		...
		<div>
			...
			<div class="block2"><p class="head">메인보드 규격</p></div>
			<div class="block3"><p class="head">파워 규격</p></div><br>
			<?php
			$conn = db_connect();
			...
			$query = "SELECT * FROM pcdb.case ORDER BY parts_num DESC LIMIT $start_point, $list"; //첫번째 게시글부터 한페이지에 보여줄 게시글 갯수만큼 들고옴
			$result = mysqli_query($conn, $query);
			...
			?>
					<div class="perLine">
						...
						<div class="block2"><?= $rows['mainboard_std'] ?></div>
						<div class="block3"><?= $rows['power_std'] ?></div>
					</div>
			<?php
				}
			}
